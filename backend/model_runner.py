...
def process_images(thermal_file, visual_file):
    ...
    if thermal_np is None or thermal_np.size == 0:
        raise FileNotFoundError("❌ لم يتم العثور على بيانات الصورة الحرارية")
    ...
    for xyxy, cls in zip(res.boxes.xyxy.cpu().numpy(), res.boxes.cls.cpu().numpy()):
        ...
        # ضبط الحدود داخل الصورة
        x1 = max(0, min(x1, W_th-1))
        x2 = max(0, min(x2, W_th-1))
        y1 = max(0, min(y1, H_th-1))
        y2 = max(0, min(y2, H_th-1))
    ...
    # حفظ النتيجة
    output_path = "output_aligned.jpg"
